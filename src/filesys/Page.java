...
public class Page implements PageInterface {
	...
	public Page(int pageId, MyFile file, boolean create) throws Throwable {
	...
	public synchronized static Integer newPageFromPool(int wantSize){
		Iterator<RemainPage> iter = pool.iterator();
		while(iter.hasNext()){
			RemainPage next = iter.next();
			if((next.remainSize << 2) < MyFile.BTREE_PAGE_SIZE){
				iter.remove();
			}
	...
	public boolean inWriting() {
		return writeCount != 0;
	}*/

	public synchronized void writeOver() throws Throwable{
		//if(writeCount > 0){
		//	writeCount--;
		//}
		if(hasFlush){
