...
public class SelectScan extends OneFromScan {
	public long time;
	...
	public boolean isPushable(){
		...
		//Debug.warn("Select isPushable false!!!");
		return false;
	}
	...
	private void getNext(){
		if(skip){
			...
			while(fromScan.hasNext()){
				...
				boolean eval = expr.valuePredicate(tmp);
				if(eval){
					now = record;
					return;
				}
	...
	public void rename(String oldName, String newName) {
		...
		boolean isNull = expr.getType(fromScan.getSchema()) == java.sql.Types.NULL;
		if(isNull){
