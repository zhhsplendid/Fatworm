	...
	*/
	...
	public static Expr getExpr(Tree tree){
		...
		case FatwormParser.AND:
			return new BinaryExpr(getExpr(tree.getChild(0)), BinaryOp.AND, getExpr(tree.getChild(1)));
		case FatwormParser.OR:
			return new BinaryExpr(getExpr(tree.getChild(0)), BinaryOp.OR, getExpr(tree.getChild(1)));
			
		case FatwormParser.INTEGER_LITERAL:
			try {
				Integer number = Integer.parseInt(tree.getText());
				return new IntExpr(number);
			} catch (NumberFormatException e) {
				return new IntExpr(new BigInteger(tree.getText()));
			}
		...
		case FatwormParser.MIN:
			return new FunctionExpr(tree.getType(), getAttributeName(tree.getChild(0)));
	
		case FatwormParser.ANY:
		case FatwormParser.ALL:
			return new AllAndAnyExpr(getSelectScan(tree.getChild(2), false), 
					getExpr(tree.getChild(0)), getBinaryOp(tree.getChild(1).getText()), 
					tree.getType() == FatwormParser.ALL);
		default:
			BinaryOp op = getBinaryOp(tree.getText());
			if(op != null && tree.getChildCount() == 2){
				...
				if(op == BinaryOp.MULT){
					if(left.isConst && left.getType() == java.sql.Types.INTEGER && right instanceof IdExpr){
						return new IdTimesExpr((IdExpr)right, ((Int)left.value).value);
					}
					if(right.isConst && left instanceof IdExpr && right.getType() == java.sql.Types.INTEGER){
						return new IdTimesExpr((IdExpr)left, ((Int)right.value).value);
					}
				}
				return new BinaryExpr(left, op, right);
			}
			else if(op != null){ //but tree child count == 1 which implies numbers like -1
				Expr childExpr = getExpr(tree.getChild(0));
				if(childExpr instanceof IntExpr){
					if(((IntExpr)childExpr).data instanceof Decimal){
						Decimal data = (Decimal) ((IntExpr) childExpr).data;
						return new IntExpr(data.value.negate().toBigIntegerExact());
					}
					else{
						Int data = (Int) ((IntExpr) childExpr).data;
						return new IntExpr(- data.value);
					}
				}
				else if(childExpr instanceof FloatExpr){
					Float data = ((FloatExpr) childExpr).data;
					return new FloatExpr(- data.value);
				}
				else if(childExpr.isConst && childExpr.value instanceof Int){
					Int data = (Int)childExpr.value;
					return new IntExpr(- data.value);
				}
				else{
					return new BinaryExpr(new IntExpr(BigInteger.valueOf(0)), op, childExpr);
				}
				
			}
			else{
				return new IdExpr(getAttributeName(tree));
			}
		}
	}
	...
	public static void removeRepeat(Collection<String> to,
		...
		for(String s1: from){
			for(String s2: to){
				if(s1.toLowerCase().endsWith(s2.toLowerCase()) || s2.toLowerCase().endsWith(s1.toLowerCase())){
					tmp.add(s1);
				}
			}
		}
	...
	public static<T> int collectionHashCode(List<T> cols) {
		int hash = 0;
		for(T o: cols){
			hash ^= o.hashCode();
		}
